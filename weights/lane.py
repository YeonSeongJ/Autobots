...
def convertLightness(position):
    ...
    if position == 0:
        B += (LIGHTNESS_LEFT[0] - bias) * (LIGHTNESS_LEFT[0] - bias) / 100
        G += (LIGHTNESS_LEFT[1] - bias) * (LIGHTNESS_LEFT[1] - bias) / 100
        R += (LIGHTNESS_LEFT[2] - bias) * (LIGHTNESS_LEFT[2] - bias) / 100
        # print('LIGHTNESS_LEFT :', LIGHTNESS_LEFT)
    elif position == 1:
        B += (LIGHTNESS_RIGHT[0] - bias) * (LIGHTNESS_RIGHT[0] - bias) / 100
        G += (LIGHTNESS_RIGHT[1] - bias) * (LIGHTNESS_RIGHT[1] - bias) / 100
        R += (LIGHTNESS_RIGHT[2] - bias) * (LIGHTNESS_RIGHT[2] - bias) / 100
