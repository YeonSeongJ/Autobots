...
def GetLine(frame, position):
    ...
    for i in range(12):
        ...
        if position == 0:
            BGR = cv2.mean(frame[5 * i:5 * (i + 1), 5 * i : 5 * (i + 1)])
        else:
            BGR = cv2.mean(frame[60 - 5 * (i + 1):60 - 5 * i, 5 * i : 5 * (i + 1)])
        # BGRs = (BGR[0] + BGR[1] + BGR[2]) / 3
        if BGR[2] - BGR[1] < weights and BGR[2] - BGR[1] < weights:
            if position == 0:
    ...
    cv2.putText(frame, str(steering_level), (int(WIDTH / 2), 20), cv2.FONT_HERSHEY_PLAIN, 2, (0, 255, 0), 3, cv2.LINE_AA)
